...
ssh %opsiuser%@%opsiserver% "opsi-package-manager -l | grep -E '^ *%pkgdelete% ' && echo '[OK] Paket gefunden' || echo '[INFO] Paket nicht in Liste gefunden'"

echo.
echo Schritt 2: Entferne Paket aus OPSI (ohne Terminal-UI)...
ssh %opsiuser%@%opsiserver% "TERM=dumb opsi-package-manager --remove %pkgdelete% --depot-id backup.paedml-linux.lokal 2>&1 | grep -v 'curses' | grep -v 'setupterm' || echo '[INFO] Versuche alternative Methode...'"

echo.
echo Schritt 3: Alternative: Direkte Backend-Loeschung...
ssh %opsiuser%@%opsiserver% "opsi-admin -d method product_delete %pkgdelete% 2>/dev/null && echo '[OK] Aus Backend entfernt' || echo '[INFO] Nicht im Backend oder bereits entfernt'"
ssh %opsiuser%@%opsiserver% "opsi-admin -d method productOnDepot_deleteObjects \"[]\" productId=%pkgdelete% 2>/dev/null || true"

echo.
echo Schritt 4: Loesche Depot-Ordner (falls noch vorhanden)...
...
echo.
echo Schritt 5: Loesche Workbench-Dateien...
