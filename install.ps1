...
REM Convert control file to UTF-8 for OPSI compatibility
powershell -Command "$content = Get-Content '%pkgdir%\OPSI\control' -Encoding Default; $content | Set-Content '%pkgdir%\OPSI\control' -Encoding UTF8"
...
REM Convert opsiscripts to UTF-8 for OPSI compatibility
powershell -Command "$content = Get-Content '%pkgdir%\CLIENT_DATA\setup.opsiscript' -Encoding Default; $content | Set-Content '%pkgdir%\CLIENT_DATA\setup.opsiscript' -Encoding UTF8"
powershell -Command "$content = Get-Content '%pkgdir%\CLIENT_DATA\uninstall.opsiscript' -Encoding Default; $content | Set-Content '%pkgdir%\CLIENT_DATA\uninstall.opsiscript' -Encoding UTF8"
...
ssh -o ConnectTimeout=10 %opsiuser%@%opsiserver% "result=$(opsi-admin -d method hostControl_start 2>/dev/null); sent=$(echo \"$result\" | grep -c '\"result\": \"sent\"'); failed=$(echo \"$result\" | grep -c '\"error\":'); echo \"WOL packets sent: $sent\"; if [ $failed -gt 0 ]; then echo \"Errors: $failed\"; fi"
echo.
