...
echo.
echo Schritt 2: Entferne Paket aus OPSI Backend...
ssh %opsiuser%@%opsiserver% "opsi-admin -d method product_delete %pkgdelete% 2>&1 | grep -v 'Traceback' && echo '[OK] Produkt geloescht' || echo '[INFO] Produkt nicht vorhanden oder bereits geloescht'"

echo.  
echo Schritt 3: Entferne Depot-Zuordnungen...
ssh %opsiuser%@%opsiserver% "opsi-admin -d method productOnDepot_delete %pkgdelete% backup.paedml-linux.lokal 2>&1 | grep -v 'Traceback' || echo '[INFO] Keine Depot-Zuordnung vorhanden'"

echo.
echo Schritt 4: Entferne Client-Zuordnungen...
ssh %opsiuser%@%opsiserver% "for client in \$(opsi-admin -d method host_getIdents 2>/dev/null | grep -v backup | tr -d '[]\",' | xargs); do opsi-admin -d method productOnClient_delete %pkgdelete% \$client 2>/dev/null; done; echo '[OK] Client-Zuordnungen entfernt'"
